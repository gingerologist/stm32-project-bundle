...
20260822 硬件修改（丝印v3.1），但本文档未更新，修改部分参见revision-260822.md；

20260917 本文档修改，硬件未改；修改内容仅在下面的“20260917修改”部分，其它部分未改动；新版硬件增加了连接器，但本文档图片未更新；
...
增加了需求定义的新模式，文档和代码打印中均称之为daily模式。

仍然通过之前的set命令定义daily，唯一去的区别是，set命令接受的三个参数，电流（1-128档位），脉冲宽度（100-1000微秒），period（50-500ms，对应2-20Hz），现允许period设置为0。在period设置为0时，系统会进入daily模式。



daily模式包含4个5分钟，每个5分钟包含30个10秒周期，每个10秒周期的前面6秒是指定周期（频率）的脉冲，后面4秒是空闲。4个5分钟对应的周期（频率）分别是333ms（3Hz），333ms（3Hz），100ms（10Hz），和50ms（20Hz）。



其它命令和之前一样，包括start和stop，即daily模式也是可以手动启动和停止的，save命令也可以保存daily模式的设置。对于show命令，如果period是0，会打印上述pattern的简单描述。



系统仍然支持保存的config可以在上电后自动启动，在启动前增加了打印并且有10秒钟延迟。



一次自动启动执行daily模式的打印输出如下。观察周期中的频率正确可以用示波器观察板上测试点CH0的电平，该测试点是启动运放时的使能，低电平有效，即看到的信号是从3.3V到0V下降沿的负脉冲。示波器水平每格单位可以设置为100ms，这样比较容易看出333ms，100ms和50ms的周期正确与否。
