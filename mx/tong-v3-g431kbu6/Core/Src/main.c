...
osMessageQId configPendingQueueHandle;
osMessageQId configIdleQueueHandle;
/* USER CODE BEGIN PV */
...
int main(void)
{
  ...
  /* Create the queue(s) */
  /* definition and creation of configPendingQueue */
  osMessageQDef(configPendingQueue, 4, uint32_t);
  configPendingQueueHandle = osMessageCreate(osMessageQ(configPendingQueue), NULL);

  /* definition and creation of configIdleQueue */
  osMessageQDef(configIdleQueue, 2, uint32_t);
  configIdleQueueHandle = osMessageCreate(osMessageQ(configIdleQueue), NULL);
  ...
  osThreadDef(pulseTask, StartPulseTask, osPriorityHigh, 0, 256);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : EN_POS_HV_Pin CH1_SD_Pin */
  GPIO_InitStruct.Pin = EN_POS_HV_Pin|CH1_SD_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
__weak void StartPulseTask(void const * argument)
{
